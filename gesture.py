...
while True:
    ...
    if hands:
        ...
        next_length, lineInfo_next_trak,img = detector.findDistance(lmlist[4][0:2],lmlist[17][0:2],img)
        print(next_length)
        
        if len(fings_up) == 4 and fingers[0]==0:
            reset()

        if fingers[1] == 1 and fingers[0] == 0 and fingers[2] == 0 and fingers[3] == 0 and fingers[4] == 1:
            pressed = 0
            print('in play-pause')
            if fingers[0] == 0 and fingers[1] == 1 and fingers[4] == 1 and fingers[2] == 0 and fingers[3] == 0 and not ('youtube' in current_window or 'groove' in current_window or 'vlc' in current_window):
                # print('clicking play')
        ...
        if fingers[0]==1 and (fingers[1] and fingers[2] and fingers[3] and fingers[4]) == 1 and next_length<=30: 
            pressed_prev = 0
            if next_pressed==0:
                keyboard.press_and_release('next_track')
                next_pressed=1

        if fingers[0]==1 and (fingers[1] and fingers[2] and fingers[3] and fingers[4]) == 0:
            next_pressed = 0 
            if pressed_prev==0:
